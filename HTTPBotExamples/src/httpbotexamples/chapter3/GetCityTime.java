...
import httpbotexamples.util.ParsingUtil;
...
public class GetCityTime
{
  ...
  public Date getCityTime(int city) throws IOException, ParseException
  {
    ...
    String dateString = ParsingUtil.extractBetweenElements(str, "<b>", "</b>", 1);
  ...
  public void go()
  {
    try
    {
      ...
      System.out.println("**Time Cities**");
      while (!done)
      {
        String line = ParsingUtil.extractBetweenElements(str, "<li>", "</a>", count);

        if (line != null)
        {
          int cityNum = Integer.parseInt(ParsingUtil.extractBetweenElements(line, "=", "\"", 2));
          int i = line.indexOf(">");
